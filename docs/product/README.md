...
A **learning-oriented relational database** in Java. Clients send SQL over TCP; the server lexes, parses, analyses, plans, and executes through a **Volcano-style pull executor** over **page-backed heaps** (`.ibd`), **B+ tree indexes** (`.idx`), JSON **catalog** files, and a **write-ahead log (WAL)**.
